import { Component, ElementRef, ViewChild, OnInit, OnDestroy, NgZone, ChangeDetectorRef, Output, EventEmitter } from '@angular/core';
import { CommonModule, Location } from '@angular/common';
// @ts-ignore
import * as pbi from 'powerbi-client';
import { PowerbiService } from '../../powerbi.service';
import { HttpClient } from '@angular/common/http';
import { HttpConnecrService } from '@org-kona-ai/shared';
...
@Component({
  ...
  providers: [HttpConnecrService],
  templateUrl: './visualization.component.html',
  styleUrl: './visualization.component.scss'
})
export class VisualizationComponent implements OnInit, OnDestroy {
  activeTab: string = 'visualization';
  @ViewChild('reportContainer', { static: true }) reportContainer!: ElementRef;

  @Output() pagesLoaded = new EventEmitter<any[]>();
  @Output() pageChanged = new EventEmitter<any>();

  // Method to be called from parent to navigate to a specific page
  ...
  constructor(
    ...
    private httpConnector: HttpConnecrService
  ) {}

  private readonly reportId = POWERBI_CONFIG.reports.visualization.reportId;
  private readonly embedUrl = POWERBI_CONFIG.reports.visualization.embedUrl;
  ...
  loadDashboard(): void {
    ...
    this.hasError = false;
    this.embedReport({});
    // Get embed info from your backend service
    // this.powerbiService.getEmbedInfo().subscribe({
    //   next: (embedInfo) => {
    //     this.embedReport(embedInfo);
    //   },
    //   error: (error) => {
  ...
  private embedReport(embedInfo: any): void {
    ...
    //     "client_id": "6f5fa4d0-5ee1-4bef-81d7-9b7dafdb154a",
    //     "client_secret": "yhd8Q~HefsSxXEzsezq.KhBIOZIHSDH3OQvRjbBN",
    //     "scope": "https://analysis.windows.net/powerbi/api/.default"
    // }
    // //microsoft restapi url for token
    //   this.http.post('https://login.microsoftonline.com/7b41559b-fcd6-4666-8562-f372c0f8f654/oauth2/v2.0/token', payload).subscribe((res: any) => {
    //     console.log(res);
  ...
  ngAfterViewInit() {
    //this.httpConnector.get('http://localhost:8000/get-embed-token', {}).subscribe((res: any) => {
    ...
    this.report.on('rendered', async () => {
      try {
        ...
        this.zone.run(() => {
          ...
          this.pagesLoaded.emit(this.pages);
      ...
      try {
        this.zone.run(() => {
          ...
          if (newActive) {
            ...
            this.pageChanged.emit(newActive);
          }
        });
      } catch (err) {
